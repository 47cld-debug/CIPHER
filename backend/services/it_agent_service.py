"""
IT Agent service for handling IT-related compliance queries using ChromaDB RAG.
"""
import logging
...
class ITAgentService:
    """IT Agent service for handling IT-related compliance queries"""
    
    async def get_response(self, query: str) -> Dict:
        """
        Get IT agent response with RAG retrieval from ChromaDB
        ...
        """
        # Search ChromaDB for relevant IT documents
        relevant_chunks = []
        policy_references = []
        
        if chromadb_service.is_available():
            try:
                results = chromadb_service.search("it", query, n_results=5)
                relevant_chunks = results
                
                # Extract policy references from metadata
                for result in results:
                    metadata = result.get('metadata', {})
                    filename = metadata.get('filename', 'IT Policy')
                    if filename not in policy_references:
                        policy_references.append(filename)
            except Exception as e:
                logger.warning(f"ChromaDB search failed for IT: {e}")
        ...
        if relevant_chunks:
            ...
            policy_context = "No specific IT policies found. General IT guidance may be provided."
            logger.warning("No IT chunks retrieved from ChromaDB")
        ...
        try:
            ...
            logger.error(f"Error generating IT response: {e}")
            response_content = "I'm having trouble processing your IT question. Please contact IT support directly."
    ...
    def _check_compliance(self, query: str, context: str) -> Optional[bool]:
        ...
        compliance_keywords = [
            ...
            "violate", "allowed to", "am i allowed", "use", "install"
        ]
...
# Global instance
it_agent_service = ITAgentService()
